...
function DesCard(props) {
	...
	return (
			...
			>
				<span>
					<p className="desc"><span dangerouslySetInnerHTML = {{__html: item.desc.substr(0, 150)}}></span>{ item.desc.length > 150 ? <span>...<Link to={`/works/${item.id}`}> read more</Link></span> : "" }<br></br></p>
				</span>
				<span>
